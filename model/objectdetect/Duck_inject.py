...
for root, paths, files in os.walk(defect_img_root):
    for path in paths:
        ...
        for idx in range(len(bboxs)):
            ...
            mask[int(left_top_y - ring_width):int(left_top_y + defect_w + ring_width),
            int(left_top_x - ring_width):int(left_top_x + defect_h + ring_width)] = 255
            ...
            if mutual_infor > 0.8:
                ...
                result.append({'name': save_temp_name, 'defect_name': d_name,
                               'bbox': [left_top_x, left_top_y, left_top_x + defect_h, left_top_y + defect_w]})
